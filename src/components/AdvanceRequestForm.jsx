...
const AdvanceRequestForm = ({ data, onUpdate, activeWorkspace }) => {
                ...
                reader.onloadend = async () => {
                    try {
                        ...
                        if (json.success && json.data) {
                            ...
                            if (d.paymentPercent) onUpdate('advancePercent', d.paymentPercent.toString());
                            if (d.paymentAmount) onUpdate('advanceAmount', Number(d.paymentAmount));
                            ...
                            alert('AI trích xuất đề nghị tạm ứng thành công!');
                        } else {
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-[28px] font-black text-slate-900 tracking-tight leading-tight flex items-center gap-2">
                            Đề Nghị Tạm Ứng Hợp Đồng
                            <span className="text-[10px] bg-amber-50 text-amber-600 px-2.5 py-0.5 rounded-full font-black border border-amber-100 uppercase tracking-wider">Bước 4</span>
                        </h1>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {bankPresets.map((preset, idx) => (
                                    <button
                                        ...
                                        className="text-[10px] bg-indigo-55 text-indigo-600 border border-indigo-150 hover:bg-indigo-100/70 font-semibold px-2 py-0.5 rounded transition-colors"
                                    >
                                        {preset.label}
                                    </button>
                                ))}
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm mb-6">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <textarea
                            rows={2}
                            className="w-full px-3 py-2 bg-white border border-slate-300 rounded text-sm outline-none text-slate-700 focus:border-amber-500 focus:ring-1 focus:ring-amber-500/30"
                            value={data.basisParagraph !== undefined ? data.basisParagraph : `Căn cứ ${data.basisType || 'Hợp đồng kinh tế'} Số: ${data.contractNumber || ''} giữa ${data.partyBName || ''} (Bên Mua) và ${data.companyName || ''} (Bên Bán) về việc ${data.goodsDescription || ''}`}
                            onChange={(e) => onUpdate('basisParagraph', e.target.value)}
                        />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-slate-700 mb-1.5">Đoạn 2: Đề nghị thanh toán tạm ứng</label>
                        <textarea
                            rows={2}
                            className="w-full px-3 py-2 bg-white border border-slate-300 rounded text-sm outline-none text-slate-700 focus:border-amber-500 focus:ring-1 focus:ring-amber-500/30"
                            value={data.requestParagraph !== undefined ? data.requestParagraph : `Vậy kính đề nghị Quý công ty thanh toán tạm ứng ${data.advancePercent || 0}% giá trị ${(data.basisType || 'Hợp đồng kinh tế').toLowerCase()} (đã bao gồm VAT) ngay sau khi ${(data.basisType || 'Hợp đồng kinh tế').toLowerCase()} được hai bên xác nhận, giá trị cụ thể như sau:`}
                    ...
                    <div>
                        ...
                        <textarea
                            rows={2}
                            className="w-full px-3 py-2 bg-white border border-slate-300 rounded text-sm outline-none text-slate-700 focus:border-amber-500 focus:ring-1 focus:ring-amber-500/30"
                            value={data.bankIntroduction !== undefined ? data.bankIntroduction : `Để đảm bảo tiến độ xử lý đơn hàng, kính đề nghị Quý công ty tạm ứng kịp thời cho chúng tôi số tiền trên vào tài khoản sau:`}

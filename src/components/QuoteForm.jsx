...
const QuoteForm = ({ data, onUpdate, onUpdateItem, onAddItem, onRemoveItem, activeWorkspace }) => {
    const [isExtracting, setIsExtracting] = useState(false);
    const [activeTierItemId, setActiveTierItemId] = useState(null);
    const [customers, setCustomers] = useState([]);
    const fileInputRef = useRef(null);

    // Tải danh bạ khách hàng từ database dựa vào công ty hiện tại
    React.useEffect(() => {
    ...
    // Các mẫu điều khoản thương mại sẵn có để chọn nhanh
    const termsPresets = [
        {
            label: 'Mẫu 30/70 (Tiêu chuẩn)',
            text: "Điều Khoản Thương Mại:\n1. Thanh toán: Tạm ứng 30% giá trị báo giá ngay sau khi đặt hàng. 70% còn lại thanh toán bằng chuyển khoản sau khi bên mua nhận đủ hàng và chứng từ hợp lệ.\n2. Giao hàng: Giao hàng trong vòng 7-10 ngày kể từ ngày tạm ứng và duyệt mẫu thiết kế.\n3. Địa điểm giao hàng: Tại kho bên mua hoặc theo thỏa thuận.\n4. Báo giá có hiệu lực trong vòng 30 ngày kể từ ngày lập."
        },
        {
            label: 'Mẫu 50/50 (Dự án)',
            text: "Điều Khoản Thương Mại:\n1. Thanh toán: Tạm ứng 50% ngay sau khi ký hợp đồng/chấp thuận báo giá. 50% còn lại thanh toán trong vòng 7 ngày sau khi bàn giao nghiệm thu.\n2. Giao hàng: Thời gian sản xuất từ 15-20 ngày kể từ ngày nhận tạm ứng đợt 1.\n3. Địa điểm giao hàng: Giao hàng tận nơi tại địa chỉ bên mua chỉ định.\n4. Báo giá có hiệu lực trong vòng 15 ngày kể từ ngày lập."
        },
        {
            label: 'Giao ngay (Có sẵn)',
            text: "Điều Khoản Thương Mại:\n1. Thanh toán: Thanh toán 100% bằng chuyển khoản hoặc tiền mặt ngay khi giao nhận hàng.\n2. Giao hàng: Hàng có sẵn trong kho, giao ngay trong vòng 24h-48h sau khi nhận xác nhận đơn hàng.\n3. Phí vận chuyển: Đơn giá đã bao gồm phí vận chuyển nội thành Hà Nội.\n4. Báo giá có hiệu lực trong vòng 7 ngày kể từ ngày lập."
        },
        {
            label: 'Đồng phục may đo riêng',
            text: "Điều Khoản Thương Mại:\n1. Thanh toán: Đặt cọc 40% giá trị đơn hàng để tiến hành sản xuất. Thanh toán 60% còn lại trong vòng 5 ngày sau khi giao nhận và ký biên bản bàn giao.\n2. Duyệt mẫu: Bên bán cung cấp 01 mẫu thực tế để bên mua duyệt chất liệu và phom dáng trước khi sản xuất hàng loạt.\n3. Bảo hành: Bảo hành đường may, cúc áo trong vòng 3 tháng kể từ ngày giao hàng.\n4. Báo giá có hiệu lực trong vòng 30 ngày kể từ ngày lập."
        }
    ];
    ...
    return (
        <div className="p-6 lg:p-8 max-w-5xl mx-auto pb-24">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-[28px] font-black text-slate-900 tracking-tight leading-tight flex items-center gap-2">
                            Báo Giá Hình Ảnh
                            <span className="text-[10px] bg-indigo-50 text-indigo-600 px-2.5 py-0.5 rounded-full font-black border border-indigo-100 uppercase tracking-wider">Bước 1</span>
                        </h1>
                        <p className="text-sm text-slate-500 mt-1 max-w-md">Khởi tạo và gửi báo giá thương mại sơ bộ cho khách hàng.</p>
                    </div>
                    
                    {/* AI Extraction Button */}
                    <div className="flex items-center gap-3">
                        <input type="file" accept=".docx,.txt,.pdf,image/*" ref={fileInputRef} className="hidden" onChange={async (e) => {
                            ...
                            try {
                                if (file.type.startsWith('image/') || file.type === 'application/pdf') {
                                    ...
                                    reader.onloadend = async () => {
                                        try {
                                            ...
                                            if (json.success && json.data) {
                                                ...
                                                alert('Đã trích xuất và điền dữ liệu hình ảnh/PDF thành công!');
                                            } else {
                                                alert('Lỗi từ AI: ' + json.message);
                                            }
                                        } catch (err) {
                                            alert('Lỗi khi gọi AI trích xuất file: ' + err.message);
                                        } finally {
                                    ...
                                    if (json.success && json.data) {
                                        ...
                                        alert('Đã trích xuất và điền dữ liệu văn bản thành công!');
                                    } else {
                                        alert('Lỗi từ AI: ' + json.message);
                                    }
                                    setIsExtracting(false);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    <div className="col-span-1 space-y-4">
                        <div>
                            ...
                            <TaxLookupInput
                                value={data.companyTax}
                                onChange={(val) => onUpdate('companyTax', val)}
                                onFound={(result) => {
                                    if (result.name && !data.companyName) onUpdate('companyName', result.name);
                                    if (result.address && !data.companyAddress) onUpdate('companyAddress', result.address);
                                }}
                            />
                        </div>
                        <div className="border-l pl-4 border-slate-200 space-y-4">
                            <div>
                                <Label className="mb-1.5 block">Người lập báo giá (Góc phải dưới)</Label>
                                <Input
                                    value={data.preparedBy || ''} onChange={(e) => onUpdate('preparedBy', e.target.value)} placeholder="Tên NV"
                                />
                            </div>
                            <div className="flex gap-2">
                                <Input
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                    <div className="col-span-4">
                        <Label className="mb-1.5 block font-bold text-indigo-600">Chọn từ Danh bạ Khách hàng (Database)</Label>
                        <select
                            className="w-full text-sm rounded-md border border-slate-200 bg-white p-2.5 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-medium text-slate-700"
                            onChange={(e) => {
                                const selectedId = e.target.value;
                                if (!selectedId) return;
                                const customer = customers.find(c => c.id === selectedId);
                                if (customer) {
                                    onUpdate('customerName', customer.fullName);
                                    onUpdate('customerAddress', customer.address || '');
                                    onUpdate('representative', customer.representative || '');
                                    onUpdate('customerPhone', customer.phone || '');
                                    onUpdate('customerTax', customer.taxCode || '');
                                }
                            }}
                            defaultValue=""
                        >
                            ...
                            {customers.map(c => (
                                <option key={c.id} value={c.id}>
                                    {c.fullName} {c.shortName ? `(${c.shortName})` : ''} - {c.representative || 'Chưa có người đại diện'}
                                </option>
                            ))}
                        </select>
                    </div>
                    <div className="col-span-4">
                        <Label className="mb-1.5 block">Mã số thuế Khách hàng (tra cứu tự động)</Label>
                        <TaxLookupInput
                            value={data.customerTax || ''}
                            onChange={(val) => onUpdate('customerTax', val)}
                            onFound={(result) => {
                                if (result.name) onUpdate('customerName', result.name);
                                if (result.address) onUpdate('customerAddress', result.address);
                            }}
                            placeholder="Nhập MST khách hàng để tự động điền tên và địa chỉ..."
                        />
                    </div>
                    <div className="col-span-2">
                        <Label className="mb-1.5 block">Tên Đơn vị/Công ty Khách hàng</Label>
                        <Input
                            value={data.customerName || ''} onChange={(e) => onUpdate('customerName', e.target.value)}
                        />
                    </div>
                    <div className="col-span-2">
                        <Label className="mb-1.5 block">Người nhận / Đại diện Khách hàng (KÍNH GỬI)</Label>
                        <Input
                            value={data.representative || ''} onChange={(e) => onUpdate('representative', e.target.value)}
                        />
                    </div>
                    <div className="col-span-4">
                    ...
                    <div className="col-span-2">
                        <Label className="mb-1.5 block">SĐT Khách hàng (Đồng bộ hợp đồng)</Label>
                        <Input
                            value={data.customerPhone || ''} onChange={(e) => onUpdate('customerPhone', e.target.value)}
                        />
                    </div>
                    ...
                    <div className="col-span-1 md:col-span-2 lg:col-span-4 border-t pt-4 mt-2">
                        <Label className="mb-2.5 block font-bold text-indigo-600">Mẫu Báo giá / Loại Báo giá</Label>
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                            {[
                                { id: 'standard', name: 'Mẫu Tiêu chuẩn', desc: 'Có hình ảnh, cột chi tiết tiêu chuẩn' },
                                { id: 'minimal', name: 'Mẫu Rút gọn / Dịch vụ', desc: 'Bố cục tinh gọn, ẩn cột hình ảnh' },
                                { id: 'visual', name: 'Mẫu Cataloge Hình ảnh', desc: 'Dạng lưới card, hình ảnh lớn nổi bật' },
                                { id: 'corporate', name: 'Mẫu Hóa đơn thương mại', desc: 'Bố cục hóa đơn chính thống xanh indigo' }
                            ].map(tpl => (
                                <button
                                    key={tpl.id}
                                    type="button"
                                    className={`p-3 text-left rounded-xl border transition-all flex flex-col justify-between h-20 outline-none ${data.templateType === tpl.id || (!data.templateType && tpl.id === 'standard') ? 'border-indigo-600 bg-indigo-50/50 shadow-sm ring-1 ring-indigo-500' : 'border-slate-200 hover:border-slate-300 bg-white'}`}
                                    onClick={() => onUpdate('templateType', tpl.id)}
                                >
                                    <span className="font-bold text-xs text-slate-800">{tpl.name}</span>
                                    <span className="text-[10px] text-slate-400 leading-tight mt-1">{tpl.desc}</span>
                                </button>
                            ))}
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 gap-6">
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[310px] overflow-hidden">
                    <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {termsPresets.map((preset, idx) => (
                                <button
                                    key={idx}
                                    type="button"
                                    onClick={() => onUpdate('terms', preset.text)}
                                    className="text-[10px] bg-indigo-50 text-indigo-600 border border-indigo-150 hover:bg-indigo-100/70 font-semibold px-2.5 py-1 rounded transition-colors"
                                >
                                    {preset.label}
                                </button>
                            ))}

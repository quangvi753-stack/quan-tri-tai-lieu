...
export default function CustomerManager({ currentUser, onCreateDocument }) {
  ...
  const { loading: taxLoading, error: taxError, lookupTax, clearError: clearTaxError } = useTaxLookup();
      ...
      <div className="flex-1 bg-slate-50 flex flex-col h-full overflow-y-auto">
        {!selectedCustomer && !isEditing ? (
          ...
          <div className="max-w-4xl w-full mx-auto p-8">
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            onKeyDown={async e => {
                              if (e.key === 'Enter') {
                                e.preventDefault();
                                const result = await lookupTax(formData.taxCode);
                                if (result) {
                                  setFormData(prev => ({
                                    ...prev,
                                  ...
                                  setTimeout(() => setTaxLookupSuccess(false), 4000);
                                }
                              }
                            }}
                            className={`flex-1 p-2.5 rounded-lg text-sm outline-none border transition-all ${
                              taxLookupSuccess ? 'border-emerald-400 ring-1 ring-emerald-300 bg-emerald-50/30' :
                              taxError ? 'border-red-300 ring-1 ring-red-200' :
                          ...
                          <button
                            type="button"
                            disabled={taxLoading || !formData.taxCode || formData.taxCode.trim().length < 8}
                            onClick={async () => {
                              const result = await lookupTax(formData.taxCode);
                              if (result) {
                                setFormData(prev => ({
                          ...
                          >
                            ...
                            {taxLoading ? 'Đang tìm...' : taxLookupSuccess ? 'Đã tìm!' : 'Tra cứu'}
                          </button>
                        </div>
                        {taxError && (
                          <div className="text-xs text-red-500 bg-red-50 px-2 py-1.5 rounded-md border border-red-100 flex items-center gap-1">
                            <span className="material-symbols-outlined text-[13px]">error</span>
                            {taxError}
                          </div>
                        )}
                        {taxLookupSuccess && (
                          <div className="text-xs text-emerald-600 bg-emerald-50 px-2 py-1.5 rounded-md border border-emerald-100 flex items-center gap-1">
                            <span className="material-symbols-outlined text-[13px]">check_circle</span>
                            Đã điền tên công ty và địa chỉ!
                          </div>
                        )}
                      </div>
                    ) : (
                      <input name="taxCode" value={formData.taxCode} onChange={handleChange} disabled className="w-full p-2.5 rounded-lg text-sm outline-none bg-slate-50 text-slate-800 font-medium" />
                    )}
                  </div>
                  {/* Tên đầy đủ */}
            ...
            {!isEditing && selectedCustomer && currentUser?.role !== 'viewer' && (
              <div>
